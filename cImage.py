...
class ImageWin(tk.Canvas):
    ...
    def captureClicks(self,n=1):
        """When the Mouse is clicked n times, close the window and exit"""
        mouseClicks = []
        while n > 0 :
            (x,y)=self.getMouse()
...
class AbstractImage(object):
    ...
    def getTkPixel(self,x,y):
        ...
        pix = self.im.get(x,y)
        # Strangely, on Mac pix is a string, while on PCs it is a triple
        if type(pix) == str :
            p = [j for j in pix.split()]
        else:
            p = pix
        return Pixel(int(p[0]),int(p[1]),int(p[2]))
